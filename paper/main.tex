...
\title{A finite certificate for the simple six-vertex\\[0.3em]
Krenn--Gu obstruction over arbitrary integral domains}
...
\begin{abstract}
We give a finite, weight-free certificate for the \emph{simple}
six-vertex case of the Krenn--Gu obstruction: for every number of
colors $D\ge 3$ there is no bi-colored weighted \emph{simple} graph on
$6$ vertices --- at most one edge per unordered pair, one ordered
endpoint-color pair per edge, bichromatic edges allowed --- in which
all $D$ monochromatic inherited vertex colorings have unit weight while
every other vertex coloring has weight zero. For $D=3,4,5$ the proof
reduces the statement to a purely combinatorial finite search: a
complete enumeration of the $5{,}610$, $2{,}160$ and $720$ possible
monochromatic-class configurations respectively, collapsed to $24$,
$5$ and $1$ orbit representatives under the symmetric group $S_6$,
followed by an exhaustive check of all $2{,}033{,}610$, $4{,}917$ and
$1$ edge-color assignments --- every one of which is eliminated by one
of two elementary necessary conditions; for $D\ge 6$ the statement
follows from a one-line edge-counting argument ($3D\le 15$). Because
the elimination is weight-free, the result holds over any integral
domain, in particular over $\mathbb C$, $\mathbb R$, $\mathbb Z$, and
for weights in $\{-1,0,1\}$. To our knowledge, the $D=3$ obstruction in
this simple six-vertex model has not previously been established over
arbitrary integral domains. The certificate is machine-reproducible
(seconds of computation) and has been independently re-verified by
fresh implementations and by an independent SAT encoding. The
unrestricted EqSystemN benchmark uses a broader model and is outside
...
shows that in the simple model the maximum dimension at $n=6$ is at
most $2$, the conjectured value. Over
\emph{nonnegative} real weights the conjecture is fully settled
(Bogdanov, see \cite{mathoverflow}). The DeepMind formal-conjectures
benchmark records formal statements for the unrestricted
equation-system model; several cases have recently received
independent formal proofs, as discussed below
\cite{alphaproof, formalconjectures}.

In this note we treat the case $n=6$ in the \emph{restricted simple
model} for every number of colors $d\ge 3$; we make no claim about the
unrestricted (multigraph) model. Recent formal work has resolved the
unrestricted EqSystemN model for $n=6$, $D=3$ over $\mathbb C$
\cite{kg63cert}; a public Lean proof establishes the all-even integer
cases, including $n=6$, $D=3$ over $\mathbb Z$ \cite{kgallevenint}; and
a sharp bound $D\le N-2$ over arbitrary integral domains for even
$N\ge 6$ rules out $D\ge 5$ at $n=6$ \cite{kgsharpbound}. For $D=3$, we
have found no public result establishing the corresponding simple
six-vertex obstruction over arbitrary integral domains in full.
Our main result is:
...
Consequently, in the simple model, no bi-colored graph on $6$ vertices
admits a monochromatic configuration of dimension at least $3$ over any
integral domain. Two features deserve emphasis:
...
The unrestricted EqSystemN model of the DeepMind
\emph{formal-conjectures} benchmark is broader and is outside the scope
of this note (Section~\ref{sec:consequences}).
...
The DeepMind \emph{formal-conjectures} benchmark encodes the
unrestricted EqSystemN model, in which independent weights are
available for the different endpoint-colour types. The theorem proved
here concerns the simple-model restriction and therefore does not by
itself settle the unrestricted benchmark declarations. Several
coefficient-domain cases have recently received independent external
formal proofs \cite{kg63cert, kgallevenint, kgsharpbound}; the present
certificate should instead be viewed as a certificate for the
simple-model restriction, which is the natural first target for a
future Lean formalization of the enumeration presented here.

\section{Conclusion and open problems}\label{sec:open}

We have certified that in the simple model no six-vertex bi-colored
graph admits a monochromatic configuration of dimension $D\ge 3$, over
any integral domain. The following directions remain:
...
the Lean kernel) would certify the simple-model restriction computed
here; the unrestricted benchmark statements of
Section~\ref{sec:consequences} require a separate treatment.
\end{enumerate}
...
including OpenAI's ChatGPT and local AI tooling backed by the DeepSeek API,
which contributed to the design of the exhaustive search, the drafting of
the manuscript, and the independent verification described in
Section~\ref{sec:verify}. All mathematical claims were
...
\bibitem{kg63cert} google-deepmind/formal-conjectures, issue \#4609,
\emph{Mark eqSystem6\_no\_solution\_d3 as solved via external Lean 4
proof}
\url{https://github.com/google-deepmind/formal-conjectures/issues/4609};
external proof repository
\url{https://github.com/algal/krenn-gu-6x3-certificate}, proof commit
c04696e515e0c02be140353fb52ea60c62e827b1.

\bibitem{kgallevenint} google-deepmind/formal-conjectures, PR \#4659,
\emph{solve all-even integer cases}
\url{https://github.com/google-deepmind/formal-conjectures/pull/4659}
(public Lean proof: for even $N\ge 6$ and $D\ge 3$,
$\neg\exists W:\mathrm{WeightsN}\,N\,D\,\mathbb Z,\
\mathrm{EqSystemN}\,N\,D\,W$).

\bibitem{kgsharpbound} \emph{Sharp color bound for monochromatic quantum
graphs over arbitrary integral domains}, formal proof repository
\url{https://github.com/KitaKen1/monochromatic-quantum-graph-sharp-bound-lean}
(main result: over every integral domain, even $N\ge 6$, any EqSystemN
solution satisfies $D\le N-2$).
...
report and certificate}, \url{https://github.com/cho-leung/krenn-gu-n6}.
