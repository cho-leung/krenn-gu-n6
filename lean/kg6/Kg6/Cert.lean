...
@[noinline] def relabelPM (p : List Nat) (m : Nat) : Nat :=
...
@[noinline] def relabelT (p : List Nat) (T : Triple) : Triple :=
...
@[noinline] def canonKey (T : Triple) : Triple :=
  perms6Lit.foldl (fun best p => let k := relabelT p T; if keyLt k best then k else best) T
